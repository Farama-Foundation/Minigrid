import time
import gym
from gym_minigrid.agents import *
import logging

from gym_minigrid.agents.StopGoModel.stopGoPed import StopGoPed
from gym_minigrid.agents.StopGoModel.stopGoVehicle import StopGoVehicle
env = gym.make('TwoLaneRoadEnv60x80-v0')       
env.reset()

v1 = StopGoVehicle(1, (14, 20), (20, 29), Direction.South, 5, 2, 1, 1)
v2 = StopGoVehicle(2, (14, 35), (20, 43), Direction.South, 5, 4, 1, 1)
v3 = StopGoVehicle(3, (39, 60), (45, 69), Direction.North, 5, 5, 1, 1)
p1 = StopGoPed(id=1, position=(0, 42), direction=Direction.East, maxSpeed=4, speed = 3)
p2 = StopGoPed(id=1, position=(59,44), direction=Direction.West, maxSpeed=4, speed = 3)
p3 = StopGoPed(id=1, position=(5,5), direction=Direction.South, maxSpeed=4, speed = 3)
p4 = StopGoPed(id=1, position=(55,60), direction=Direction.North, maxSpeed=4, speed = 3)

env.addVehicleAgent(v1)
env.addVehicleAgent(v2)
env.addVehicleAgent(v3)
env.addPedAgent(p1)
env.addPedAgent(p2)
env.addPedAgent(p3)
env.addPedAgent(p4)

for i in range(110):

    obs, reward, done, info = env.step(None)
    
    if done:
        "Reached the goal"
        break

    env.render()

    if i % 10 == 0:
        logging.info(f"Completed step {i+1}")

    time.sleep(0.05)